...
class _BatchRegistrationScreenState extends State<BatchRegistrationScreen> {
  ...
  Future<void> _handleSubmit(String village) async {
    ...
    final ashaId = provider.asha?.uid ?? 'unknown';

    final infants = List.generate(_nameControllers.length, (i) {
      final name = _nameControllers[i].text.trim();
      final ageStr = _ageControllers[i].text.trim();
      final ageMonths = int.tryParse(ageStr) ?? 0;
      
      // Calculate a dummy DOB based on age in months
      final dob = DateTime.now().subtract(Duration(days: ageMonths * 30));

      return Child(
        childId: '', // Will be generated by Firestore service
        name: name,
        dob: dob,
        gender: _genders[i],
        parentName: 'Unknown', // Placeholder
        parentPhone: 'Unknown', // Placeholder
        village: village,
        createdBy: ashaId,
        status: 'new',
    ...
    await provider.registerInfants(infants);
